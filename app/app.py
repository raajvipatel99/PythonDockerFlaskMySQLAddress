...
def record_view(address_id):
    ...
    result = cursor.fetchall()
    return render_template('view.html', title='View Form', address=result[0])
...
def form_update_post(address_id):
    cursor = mysql.get_db().cursor()
    inputData = (request.form.get('fname'), request.form.get('lname'), request.form.get('ad'),
                 request.form.get('fldCountry'), request.form.get('fldAbbreviation'),
                 request.form.get('fldCapitalStatus'), request.form.get('fldPopulation'), address_id)
    sql_update_query = """UPDATE addresses t SET t.fldName = %s, t.fldLat = %s, t.fldLong = %s, t.fldCountry = 
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO addresses (fldName,fldLat,fldLong,fldCountry,fldAbbreviation,fldCapitalStatus,fldPopulation) VALUES (%s, %s,%s, %s,%s, %s,%s) """
